...
5. Make `default_env_candidates` honour `TONE_OF_VOICE_FALLBACK_ENV` and document the sibling-repo fallback inline.
...
- Worktree path resolution for the sibling-repo fallback already pre-dated this change, so the new env-var override is additive only — no behaviour change in the default path.
