...
8. The fallback path for external `.env` reuse in `default_env_candidates` must be overridable via an environment variable so it does not silently couple the project to a fixed local layout.
