...
- **Bootstrap chicken-and-egg.** The new gate would fail on its own introducing PR if it tried to validate a Codex review for changes that include the gate itself. Mitigated by the bootstrap-skip step, which is the same pattern tone-of-voice already uses for the existing claude-only `ai-review.yml`.
